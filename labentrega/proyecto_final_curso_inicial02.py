...
def alta(producto, descripcion, tree):  
    ...
    try:
        ...
        # Obtener el número total de filas en el Treeview
        # total_filas = tree.index("end")

        # Desplazarse hasta la última fila (el nuevo producto)
        # tree.see(total_filas -1)
        #'tree.see(3)


                # Obtén todos los elementos (filas) en el Treeview
        elementos = tree.get_children()
        
        # Verifica si hay elementos en el Treeview
        if elementos:
            # Obtiene el último elemento (fila)
            last_element = elementos[-1]
            
            # Desplázate hasta la última fila
...
def seleccionar_fila(event):
    ...
    if seleccion:  # Verifica si hay una fila seleccionada
        item = tree.item(seleccion[0])  # Obtiene los valores de la fila seleccionada
        # Establece los valores en los campos de carga de datos
        val_producto.set(item["values"][0])  # Producto
        val_descripcion.set(item["values"][1])  # Descripción
