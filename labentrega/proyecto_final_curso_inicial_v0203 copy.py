...
def validar_carga(producto, descripcion, stock):
...
def alta(producto, descripcion, stock, tree):  
   
    if not validar_carga(producto, descripcion, stock) == 0:
        return
   
    try:
        con=conexion()
        cursor=con.cursor()
        data=(producto, descripcion.strip(), stock)
        sql="INSERT INTO productos(producto, descripcion, stock) VALUES(?, ?, ?)"
        cursor.execute(sql, data)
        con.commit()
        print("Alta exitosa del registro con Producto: {} ,Descripción: {} y Stock: {}".format(producto, descripcion, stock))
...
def modificacion(producto, descripcion, stock, tree):  
    ...
    try:
        ...
        cursor=con.cursor()
        #mi_id = int(mi_id)
        data = (producto, descripcion, stock, mi_id)
        sql = "UPDATE productos SET producto = ?, descripcion= ?, stock=? WHERE id = ?"
        cursor.execute(sql, data)
        con.commit()        
...
val_producto, val_descripcion, val_stock = StringVar(), StringVar(), StringVar()
w_ancho = 20
...
# Botones
boton_alta=Button(root, text="Agregar", command=lambda:alta(val_producto.get(), val_descripcion.get(), val_stock.get(), tree))
boton_alta.grid(row=8, column=0)

boton_modificar=Button(root, text="Modificar", command=lambda:modificacion(val_producto.get(), val_descripcion.get(), val_stock.get(), tree))
